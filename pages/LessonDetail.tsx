...
import { evaluateBadges } from '../services/badgeService'; // Importar servicio de logros
            ...
            return (
                <div key={idx} className="w-full rounded-3xl overflow-hidden shadow-2xl border-2 border-primary/30 bg-[#0b0f14] relative group">
                   <div className="bg-[#111a22] px-4 py-2 border-b border-white/5 flex justify-between items-center z-20 relative">
                      ...
                      <a href={block.content} target="_blank" rel="noopener noreferrer" className="text-text-secondary hover:text-white transition-colors"><span className="material-symbols-outlined text-sm">open_in_new</span></a>
                   </div>
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col font-body">
      <header className="sticky top-0 z-40 bg-surface-dark border-b border-border-dark px-4 py-2 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-3">
           <button className="bg-primary/20 text-primary p-1.5 rounded-lg hover:bg-primary/30 transition-colors"><span className="material-symbols-outlined text-lg">smart_toy</span></button>
        </div>
      </header>
           ...
           {!quizOpen ? (
             <article className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                <header className="space-y-3">
                   <h2 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white leading-tight">{currentSection.title}</h2>
                </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {currentSection.interaction && (
                        <div className="p-6 bg-purple-500/5 rounded-2xl border border-purple-500/20 flex gap-4 items-start">
                            <div className="p-2 bg-purple-500 text-white rounded-xl shadow-lg shadow-purple-500/20 shrink-0"><span className="material-symbols-outlined text-lg">touch_app</span></div>
                            <div><h4 className="text-purple-500 font-black text-[10px] uppercase tracking-widest mb-1">Micro-Desafío</h4><p className="text-xs text-slate-500 dark:text-text-secondary leading-relaxed">{currentSection.interaction}</p></div>
                        </div>
                    )}
                    {currentSection.fact && (
                        <div className="p-6 bg-blue-500/5 rounded-2xl border border-blue-500/20 flex gap-4 items-start">
                            <div className="p-2 bg-blue-500 text-white rounded-xl shadow-lg shadow-blue-500/20 shrink-0"><span className="material-symbols-outlined text-lg">analytics</span></div>
                            <div><h4 className="text-blue-500 font-black text-[10px] uppercase tracking-widest mb-1">Nota Técnica</h4><p className="text-xs text-slate-500 dark:text-text-secondary leading-relaxed">{currentSection.fact}</p></div>
                        </div>
                    )}
                </div>
             ...
             <div className="max-w-4xl mx-auto py-8 space-y-8 animate-in zoom-in-95 duration-500">
                ...
                {lesson.quiz && lesson.quiz.map((q, qIndex) => (
                    <div key={qIndex} className="p-6 bg-card-dark rounded-3xl border border-border-dark space-y-4 shadow-xl relative">
                        <div className="absolute -left-3 -top-3 size-8 bg-primary text-white rounded-full flex items-center justify-center font-black text-xs border-4 border-background-dark">{qIndex + 1}</div>
                        <h3 className="text-lg font-bold text-white pl-3">{q.question}</h3>
                        <div className="space-y-2">
                            {q.options.map((opt, i) => {
                                const isSelected = userAnswers[qIndex] === i;
                                return (
                                    <button key={opt} onClick={() => handleAnswerSelect(qIndex, i)} className={`w-full p-4 rounded-xl border text-left text-sm font-bold transition-all flex justify-between items-center ${isSelected ? 'bg-primary border-primary text-white shadow-lg' : 'bg-surface-dark border-border-dark text-text-secondary hover:border-primary/50'}`}>
                                        <span>{opt}</span>
                        ...
                        {userAnswers[qIndex] !== undefined && userAnswers[qIndex] !== q.correctIndex && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-bold animate-in fade-in flex items-center gap-2"><span className="material-symbols-outlined text-base">error</span><span>Respuesta incorrecta. {q.hint}</span></div>}
                        {userAnswers[qIndex] !== undefined && userAnswers[qIndex] === q.correctIndex && <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-500 text-xs font-bold animate-in fade-in flex items-center gap-2"><span className="material-symbols-outlined text-base">check_circle</span><span>¡Correcto!</span></div>}
                    </div>
                ))}
                <div className="sticky bottom-6 bg-surface-dark/95 backdrop-blur-md p-4 rounded-2xl border border-border-dark shadow-2xl flex justify-between items-center">
                    <div className="text-xs font-bold text-text-secondary uppercase tracking-widest">{Object.keys(userAnswers).length} / {lesson.quiz.length} respondidas</div>
                    {allCorrect ? (
                         <div className="flex gap-3">
                            <button onClick={handleFinish} className="px-6 py-2.5 bg-green-500 text-white rounded-lg text-xs font-black uppercase hover:bg-green-600 transition-colors shadow-lg shadow-green-500/20 animate-bounce">¡Finalizar Módulo!</button>
                            {nextLessonId && <button onClick={handleNextLesson} className="px-5 py-2.5 bg-white/10 text-white rounded-lg text-xs font-black uppercase border border-white/20 hover:bg-white/20 transition-colors">Siguiente</button>}
                        </div>
                    ) : (
                        <button disabled className="px-6 py-2.5 bg-slate-700 text-slate-400 rounded-lg text-xs font-black uppercase cursor-not-allowed border border-slate-600">Completa para avanzar</button>
                    )}
                </div>
             </div>
           )}
